...
class FindLCA
{
	...
	Node LCA(Node root, int n1, int n2)	{
        ...
        //if both values are less than the root,then the LCA must be in the left sub-tree
        if(n1<root.data && n2<root.data)
            return LCA(root.left,n1,n2);

        //if both values are more than the root,then the LCA must be in the right sub-tree
