...
Your Task:
You have to complete the method removeDuplicates() which takes 1 argument: the head of the linked list.  Your function should
return a pointer to a linked list with no duplicate element.
...
class RemoveDuplicatesUnsortedLL
{
    ...
    public Node removeDuplicates(Node head)
    {
         ...
         while(curr!=null)
         {
			 //if already present in the hashset, delete the link
             if(visited.contains(curr.data))
                prev.next=curr.next;

             //else add in the hashset :)
             else
             {
